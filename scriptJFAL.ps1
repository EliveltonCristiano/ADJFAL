...
#function Unidades($computador){
#    $c=Invoke-Command -ComputerName NTI-035 -ScriptBlock {Get-CimInstance -Class Win32_NetworkConnection | select LocalName | format-list} | Out-String
#    $c.name | ForEach-Object  {
#        write-host "$_"
#    }
#}
...
function PSDrive($computador){
    #CONFIGURAR JANELA
     ...
     $Button_Confirmar.Add_Click( {
        ...
        if ($ListBox_usuarios.SelectedIndices -ne $null){
            ...
            if(testconnection($computador)){   
                 ...
                 $outputBox.Appendtext("`n")
                 #$cred = Get-Credential
                 $r =  Invoke-Command -ComputerName $computador -ScriptBlock {Get-CimInstance Win32_NetworkConnection | Format-Table -Property LocalName, RemoteName} | Out-String
                 #$r =  Invoke-Command -ComputerName $computador -ScriptBlock {Get-CimInstance -Class Win32_NetworkConnection | select LocalName, RemoteName | format-list} | Out-String
     ...
     $Button_Remover = New-Object System.Windows.Forms.Button 
     $Button_Remover.Location = New-Object System.Drawing.Size(310, 330) 
     $Button_Remover.Size = New-Object System.Drawing.Size(100, 20)      
     $button_Remover.text = "Remover"
     $Button_Remover.Add_Click( {
     ...
     $Form.Controls.Add($Button_Remover)

     $Button_Adicionar = New-Object System.Windows.Forms.Button 
     $Button_Adicionar.Location = New-Object System.Drawing.Size(410, 330) 
     $Button_Adicionar.Size = New-Object System.Drawing.Size(100, 20)      
     $Button_Adicionar.text = "Adicionar"
     $Button_Adicionar.Add_Click( {
     ...
     $Form.Controls.Add($Button_Adicionar)

     
     $ComboBox = New-Object System.Windows.Forms.ComboBox
     $ComboBox.Text = "Unidades"
     $ComboBox.Width = 62
     $ComboBox.Height = 100
     $ComboBox.Location = New-Object System.Drawing.Size(230, 330)
     $Form.Controls.Add($ComboBox)
...
function Impressoras($computador){
                ...
                if(testconnection($computador)){   
                    Invoke-Command -ComputerName $computador -ScriptBlock {net use LPT1} | Out-String
                    #net use LPT1:                           
                }else{
